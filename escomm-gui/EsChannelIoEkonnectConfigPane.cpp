...
{
	m_btnResetToDefaults = new wxButton( this, wxID_ANY, _("Reset to defaults").c_str());
  ...
  edFlags.Border().Expand();
	...
	m_btnRescan->SetToolTip( _("Rescan EKONNECT devices").c_str());
	...
	m_settings->AddPage( m_pnlStd, _("Generic").c_str(), true );
  ...
  edFlagsNoProportion.Proportion(0);
	...
	m_settings->AddPage( m_pnlAdvanced, _("Advanced").c_str(), false );
